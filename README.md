# Kiro Superpowers Discipline v0.8.0

把 Superpowers 的核心执行纪律改造成 Kiro 可用的 Power / Steering / Hooks / Custom Subagents。

**v0.8.0 是稳定化整理版。** 它不新增新的开发流程能力，只做一致性、文档、校验和用户体验整理；v0.2 到 v0.7 的安装、卸载和日常使用方式保持不变。
...
```text
新功能 → Feature Spec → requirements/design/tasks → worktree gate → TDD → subagent loop → review → completion contract → branch finishing
Bugfix → 复现 → 根因 → 失败测试/替代验证 → 最小修复 → review → completion contract → branch finishing
