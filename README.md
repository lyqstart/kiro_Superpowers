# Kiro Superpowers Discipline v1.3.0

把 Superpowers 的核心执行纪律改造成 Kiro 可用的 Power / Steering / Hooks / Custom Subagents。

**v1.3.0 是 Debugging Deep Techniques 版本。** 它只在 v1.2.0 基础上增强 root-cause-tracing、defense-in-depth、condition-based-waiting、multi-component diagnostics 和 3-fix architecture stop gate；安装、卸载和日常使用方式保持不变。
...
Bugfix → 复现 → root-cause-tracing → multi-component diagnostics（如适用）→ defense-in-depth 判断 → condition-based waiting（如适用）→ Task Refinement Gate → Subagent Task Packet → RED 失败测试/替代验证 → 最小修复 → review evidence → completion contract → branch finishing
...
关键词：root-cause-tracing, defense-in-depth, condition-based-waiting, multi-component diagnostics, architecture stop gate, 根因链路, 多层防护, 条件等待, 多组件诊断。


兼容关键词：Branch finishing hardening。
