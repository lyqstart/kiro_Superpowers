# Kiro Superpowers Discipline v1.4.0

把 Superpowers 的核心执行纪律改造成 Kiro 可用的 Power / Steering / Hooks / Custom Subagents。

**v1.4.0 是 Fresh Verification Evidence 版本。** 它只在 v1.3.0 基础上增强完成声明前的新鲜验证证据、verification result contract 和 completion claim hardening；安装、卸载和日常使用方式保持不变。
...
Bugfix → 复现 → root-cause-tracing → multi-component diagnostics（如适用）→ defense-in-depth 判断 → condition-based waiting（如适用）→ Task Refinement Gate → Subagent Task Packet → RED 失败测试/替代验证 → 最小修复 → review evidence → fresh verification evidence → completion contract → branch finishing
